...
scrapeBtn.onclick = () => {
    ...
    const scraperPath = path.join(__dirname, '../../services/scraper/background.js');
    const scraper = fork(scraperPath);

    scraper.on('message', (msg) => {
        // Handle both raw strings and structured messages
    ...
    scraper.on('exit', (code) => {
        if (code === 0) {
            updateOutput("✅ Productos procesados correctamente.", 'green');
            updateOutput('🔗 Ejecutando la unión de productos...');
            const mergePath = path.join(__dirname, '../../services/scraper/merge-new-products.js');
            exec(`node "${mergePath}"`, (error, stdout, stderr) => {
                updateOutput(
                    error ? `❌ Error durante la unión: ${stderr || error.message}` : `✅ ${stdout}`,
                    error ? 'red' : 'green'
                );
            });
        } else {
            updateOutput("❌ La recolección terminó con errores.", 'red');
        }
    });
};
